...
public class MobileDriver {
    ...
    public static void main(String[] args) {
		 ...
		 * dao.addMobile(new Mobile("Infinix", "Hot 12", 25000, 128)); dao.addMobile(new
		 * Mobile("Motorola", "G31", 15000, 64));
		 */
		...
		/*
		 * if (mobile != null) { mobile.setPrice(67999); dao.updateMobile(mobile); }
		 */
        // READ all
        ...
        System.out.println("\nSamsung mobiles priced below 20000:");
        List<Mobile> samsungBelow20k = dao.getSamsungMobilesBelowPrice(20000);
        for (Mobile m : samsungBelow20k) {
            System.out.println(m.getMobileId() + " | " + m.getBrand() + " | " + m.getModel() + " | " + m.getPrice() + " | " + m.getStorage() + "GB");
        }

        // 2) Get mobiles priced between 20000 and 30000 inclusive
        System.out.println("\nMobiles priced between 20000 and 30000:");
        List<Mobile> mobilesBetween20kAnd30k = dao.getMobilesInPriceRange(20000, 30000);
        for (Mobile m : mobilesBetween20kAnd30k) {
            System.out.println(m.getMobileId() + " | " + m.getBrand() + " | " + m.getModel() + " | " + m.getPrice() + " | " + m.getStorage() + "GB");
        }
        dao.close();
    }
